...
public class TarifasDepositoController : ControllerBase
{
    ...
    public async Task<TarifasDeposito> GetByNearestFecha(string fechahora, int carga, int paisregion)
    {
        try
        {
            return await _unitOfWork.TarifasDepositos.GetByNearestDateAsync(fechahora,carga,paisregion);
        }
        catch (Exception ex)
        {
            throw new Exception(ex.Message);
    ...
    [HttpGet("vistafecha/")]
    public async Task<IEnumerable<TarifasDepositoVista>> GetAllVistaByDate()
    {
        try
        {   // HH en las horas significa formato 24h.
            return await _unitOfWork.TarifasDepositos.GetAllVistaByDateAsync(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        }
        catch (Exception ex)
        {
            throw new Exception(ex.Message);
